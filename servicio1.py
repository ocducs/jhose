...
#       - Incluir mas dipositivos
#       - Incluir ultima actualización (la correspondiente a la ultima estimación del Dispositvo X)
...
def plotear(x,y,plt):
    ...
    try:
        ...
        shutil.copyfile('grafica_original.png', './static/img/grafica.png')
...
def on_message(client, userdata, msg):
    ...
    mensaje = str(msg.payload,encoding='utf-8')
    ...
    """
        ejemplo de mensaje enviado desde esp32:
    ...
    print(observacion)
    print("Enviando observacion a la Base de datos")

    insertarobservacion(hostname=observacion['nombre'],power=observacion['distancia'],rssi=observacion['rssi'],x=observacion['x'],y=observacion['y'])
    """
    void calcular() {
        float p[4]; //cociente de potencia i y sumatoria de potencia
        float s = 0;

        for (int i = 0; i < 4; i++) {
            s = s + misAps[i].getDistancia();

        }
        for (int i = 0; i < 4; i++) {
            p[i] = misAps[i].getDistancia() / s;

        }
        X = p[0] * misAps[0].x + p[1] * misAps[1].x + p[2] * misAps[2].x + p[3] * misAps[3].x;
        Y = p[0] * misAps[0].y + p[1] * misAps[1].y + p[2] * misAps[2].y + p[3] * misAps[3].y;
    ...
    #    print(e)
    #    print("hubo un error al deserializar...")
    #
